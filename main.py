#!/usr/bin/env python3
"""
Modern Graph Licensing Optimization Tool

A comprehensive tool for solving graph licensing optimization problems
using various algorithms with parameter tuning and dynamic analysis.
"""
...
def get_timestamp_suffix() -> str:
    """Generate timestamp suffix for result directories."""
    return datetime.now().strftime("%Y%m%d_%H%M%S")


def create_timestamped_path(base_path: str, command_name: str) -> Path:
    """Create timestamped directory path for results."""
...
def create_test_graph(
    ...
    **kwargs
    ...
    """Create test graph."""
    return GraphGenerator.generate_graph(
...
def cli(ctx, log_level):
    """Graph Licensing Optimization Tool."""
...
def benchmark(algorithms, graph_types, graph_sizes, iterations):
    ...
    selected_algorithms = {name: algo for name, algo in all_algorithms.items() 
        ...
        for size in graph_sizes:
            for i in range(iterations):
                ...
                for algo_name, algorithm in selected_algorithms.items():
                    ...
                    try:
                        result = benchmark_runner.run_single_test(
                            algorithm, graph, config, test_name
                        )
...
@cli.command()
@click.option("--algorithm", required=True, 
              type=click.Choice(["genetic", "simulated_annealing", "tabu_search"]))
...
def tune(algorithm, n_trials, graph_type, graph_size, metric):
    ...
    # Always create timestamped directory and save full results
...
def dynamic(algorithm, graph_type, initial_size, iterations, modification_prob, create_gif):
    ...
    # Create initial graph and config
...
def compare(algorithms, graph_type, graph_size, solo_cost, group_cost, 
    ...
    for i, result in enumerate(results_summary, 1):
        ...
        click.echo(f"{i}. {result['algorithm']}: ${result['total_cost']:.2f} "
                  f"(Solo: {result['solo_licenses']}, Groups: {result['group_licenses']}) {status}")
    
    # Create visualization
    visualizer = GraphVisualizer()
    
    # Always create timestamped output directory and path
    ...
    # Always save comparison results as JSON
    comparison_data = {
        "timestamp": datetime.now().isoformat(),
        "graph_info": {
            "type": graph_type,
