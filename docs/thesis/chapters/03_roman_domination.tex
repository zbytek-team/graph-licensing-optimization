...
\paragraph{Klasyczny RD — bez limitów.} W klasycznym modelu RD przyjmuje się, że wierzchołek z etykietą $2$ \emph{„broni”} siebie oraz dowolnie wielu sąsiadów z etykietą $0$ -- nie istnieje górny limit liczby bronionych sąsiadów \cite{Cockayne2004}. Etykieta $1$ pokrywa wyłącznie właściciela, a etykieta $0$ oznacza wierzchołek broniony przez sąsiada z etykietą $2$.
...
\subsection{Wersja pojemnościowa a RD: różnice i definicja limitu}
\label{sec:rd-capacitated}
W naszym uogólnieniu licencje grupowe mają pojemność \emph{górną} i (opcjonalnie) \emph{dolną}: dla typu $t=(c_t,m_t,k_t)$ liczba miejsc w grupie (wraz z właścicielem) musi należeć do przedziału $[m_t,\,k_t]$. Odpowiada to \emph{pojemnościowemu dominowaniu} (capacitated domination) znanemu w literaturze: wierzchołek „centrum” może obsłużyć co najwyżej określoną liczbę wierzchołków z sąsiedztwa \cite{cap_dom_ars2010,cap_dom_planar_2016}. W notacji ILP wykorzystywanej w pracy oznacza to w szczególności ograniczenia typu
\[
\sum_{v\in N[u]\cup\{u\}} y_{u,v} \;=\; s_u,\qquad m_t\,x^{(t)}_u \;\le\; s_u \;\le\; k_t\,x^{(t)}_u,\qquad \sum_{u\in N[v]\cup\{v\}} y_{u,v} = 1,
\]
gdzie $x^{(t)}_u\in\{0,1\}$ wybiera typ licencji u wierzchołku $u$, a $y_{u,v}\in\{0,1\}$ przypisuje wierzchołek $v$ do grupy właściciela $u$. W klasycznym RD warunek górny znika ($k_t=\infty$), a minimalizacja kosztu przy $c_1=1,\ c_g=2$ redukuje się do RD (por. powyższe twierdzenie).

Istotną różnicą praktyczną jest typ \emph{Duo}: dokładnie dwuosobowa grupa (wraz z właścicielem), co modelujemy jako $m_t=k_t=2$. W klasycznym RD nie ma takiego ograniczenia wielkości „obozu” wierzchołka $2$.
...
\section{Literatura o RD i wariantach}\label{sec:rd-literature}
Uzupełniając kontekst, odwołujemy się do klasycznego artykułu \emph{Roman domination in graphs} \cite{Cockayne2004} oraz wyników ekstremalnych \cite{Chambers2009Extremal}. Perspektywę parametryczną przedstawiają \cite{fernau2006paramRD,fernau2008paramRD}. Liczne uogólnienia (np. \emph{Italian/Roman\{2\}-domination}, silna/totalna dominacja rzymska) są podsumowane m.in. w nowszych przeglądach \cite{roman2_survey_2024} oraz monografii \cite{henning2013roman}. Z kolei wariant pojemnościowy jest bliski \emph{capacitated dominating set} \cite{cap_dom_ars2010,cap_dom_algo2011,cap_dom_planar_2016}.

\section{Alternatywne ujęcia poza RD}
\paragraph{Set Cover / Facility Location.} Nasz ILP ma naturę \emph{facility-location}: etykieta $2$ to „otwarcie placówki” (koszt $c_g$) z pojemnością $k_t$, etykieta $1$ to placówka o pojemności $1$ i koszcie $c_1$, a przypisania są do sąsiadów w grafie. To ujęcie ułatwia wykorzystanie znanych heurystyk i aproksymacji z lokalizacji obiektów oraz pokrycia zbiorów \cite{wolsey1998,fernau2006paramRD,cap_dom_planar_2016}.
\paragraph{Duo jako dopasowania.} Dla scenariusza z samym planem \emph{Duo} (i/lub jego przewagą cenową) sensowne są heurystyki oparte na maksymalnym dopasowaniu/b-dopasowaniu: tworzymy pary sąsiadów, minimalizując liczbę nieparzystych „resztek”, które muszą kupić licencję indywidualną. Jednak w obecności planu rodzinnego (większej pojemności) problem przestaje być czystym dopasowaniem i wraca do dominacji/Facility Location.
\paragraph{Pokrycie wierzchołkowe.} Vertex Cover pokrywa krawędzie, nie wierzchołki; nie modeluje naturalnie „grup” obejmujących wielu sąsiadów jednocześnie i ma inną funkcję celu. Może być użyte cząstkowo (np. do ograniczenia krawędzi „spornych”), ale nie zastępuje modelu dominacyjnego.
