...
# Only a single graph type: tree
GRAPH_NAMES: list[str] = ["tree"]
GRAPH_DEFAULTS: dict[str, dict[str, Any]] = {"tree": {"seed": 42}}

# Match benchmark.py sizing philosophy
...
# Only the requested license configs
LICENSE_CONFIG_NAMES: list[str] = [
    "duolingo_super",
    "roman_domination",
]

# Run all algorithms except Naive; ensure TreeDynamicProgramming is included
...
GRAPH_CACHE_DIR: str = "data/graphs_cache_trees"
...
def _cache_paths(cache_dir: str, n: int, sample: int) -> tuple[Path, Path]:
    base = Path(cache_dir) / f"n{n:04d}"
...
def _ensure_cache_for_all() -> None:
    ...
    created = 0
    for n in SIZES:
        base_params = dict(GRAPH_DEFAULTS["tree"])
        for s_idx in range(SAMPLES_PER_SIZE):
            gpath, mpath = _cache_paths(GRAPH_CACHE_DIR, n, s_idx)
            if gpath.exists() and mpath.exists():
                continue
            seed = int((base_params.get("seed", 42) or 42) + s_idx * 1009)
            params = dict(base_params)
            params["seed"] = seed
            G = generate_graph("tree", n, params)
            Path(gpath).parent.mkdir(parents=True, exist_ok=True)
            with gpath.open("wb") as f:
                pickle.dump(G, f, protocol=pickle.HIGHEST_PROTOCOL)
            meta = {"type": "tree", "n": n, "params": params, "sample": s_idx}
            with mpath.open("w", encoding="utf-8") as f:
                json.dump(meta, f, ensure_ascii=False)
            created += 1
    if created:
        print(f"graph cache: generated {created} new trees under {GRAPH_CACHE_DIR}")
    else:
        print(f"graph cache: up-to-date at {GRAPH_CACHE_DIR}")


def _load_cached_graph(n: int, sample_idx: int) -> tuple[nx.Graph, dict[str, Any]]:
    gpath, mpath = _cache_paths(GRAPH_CACHE_DIR, n, sample_idx)
...
def main() -> None:
    run_id = build_run_id("trees", RUN_ID)
        ...
        for algo_name in ALGORITHM_CLASSES:
            ...
            for n in SIZES:
                if stop_sizes:
                    break
                for s_idx in range(SAMPLES_PER_SIZE):
                    G, params = _load_cached_graph(n, s_idx)
                    ...
                    for rep in range(REPEATS_PER_GRAPH):
                        ...
                        if row.get("notes") == "timeout":
                            ...
                            status = "ERROR"
                        print(
                            f"tree        n={n:4d} s={s_idx} rep={rep} cost={row['total_cost']:.2f} "
                            f"time_ms={row['time_ms']:.2f} valid={row['valid']} {status}"
                        )
                        if is_over:
                            over_here = True
                    if over_here:
                        print(f"tree        n={n:4d} TIMEOUT stopping larger sizes for {algo_name}")
                        stop_sizes = True
                        break
