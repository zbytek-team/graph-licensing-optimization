...
class RealWorldDataLoader:
    ...
    def load_facebook_ego_network(self, ego_id: str) -> nx.Graph:
        ...
        # Dodaj węzeł ego (nie występuje w pliku edges, ale łączy się ze wszystkimi)
        ego_node = int(ego_id)
            ...
            for line in f:
                ...
                if line:
                    ...
                    if len(parts) >= 2:
                        ...
                        # Dodaj krawędzie do węzła ego
                        graph.add_edge(ego_node, node1)
                        graph.add_edge(ego_node, node2)
        
        # Wczytaj cechy węzłów jeśli dostępne
        ...
        self.logger.info(f"Załadowano Facebook ego network {ego_id}: "
                        f"{len(graph.nodes())} węzłów, {len(graph.edges())} krawędzi")
    ...
    def create_combined_facebook_network(self, max_networks: Optional[int] = None) -> nx.Graph:
        ...
        self.logger.info(f"Utworzono połączony graf Facebook: "
                        f"{len(combined_graph.nodes())} węzłów, {len(combined_graph.edges())} krawędzi")
