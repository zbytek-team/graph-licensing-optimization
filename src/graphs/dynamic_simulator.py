...
class MutationParams:
    ...
    add_nodes_prob: float = 0.1  # Prawdopodobieństwo dodania węzłów
    remove_nodes_prob: float = 0.05  # Prawdopodobieństwo usunięcia węzłów
    add_edges_prob: float = 0.15  # Prawdopodobieństwo dodania krawędzi
    remove_edges_prob: float = 0.1  # Prawdopodobieństwo usunięcia krawędzi
    max_nodes_add: int = 3  # Maksymalna liczba dodawanych węzłów
    max_nodes_remove: int = 2  # Maksymalna liczba usuwanych węzłów
    max_edges_add: int = 5  # Maksymalna liczba dodawanych krawędzi
    max_edges_remove: int = 3  # Maksymalna liczba usuwanych krawędzi
...
class DynamicNetworkSimulator:
    ...
    def _add_nodes(self, graph: nx.Graph, num_nodes: int) -> List[int]:
        ...
        for _ in range(num_nodes):
            ...
            # Dodaj losowe połączenia z istniejącymi węzłami
            if existing_nodes:
    ...
    def _rebalance_licenses(
        ...
        """
        # Zbierz węzły, które nadal istnieją w grafie
        existing_nodes = set(graph.nodes())

        # Sprawdź które grupy z poprzedniego rozwiązania są nadal valid
        valid_groups = []
        ...
        for group in old_solution.groups:
            ...
            # Sprawdź czy wszystkie węzły grupy nadal istnieją
            if not group_nodes.issubset(existing_nodes):
                continue

            # Sprawdź czy grupa jest nadal spójna (wszyscy są sąsiadami ownera)
            if self._is_group_valid(graph, group):
                valid_groups.append(group)
                uncovered_nodes -= group_nodes

        # Dla niepokrytych węzłów znajdź nowe przypisania
        if uncovered_nodes:
            # Utwórz podgraf z niepokrytymi węzłami
            subgraph = graph.subgraph(uncovered_nodes).copy()

            # Znajdź rozwiązanie dla niepokrytych węzłów
            if len(subgraph.nodes()) > 0:
                new_solution = self.rebalance_algorithm.solve(subgraph, license_types)
                valid_groups.extend(new_solution.groups)

        # Utwórz finalne rozwiązanie
    ...
    def _is_group_valid(self, graph: nx.Graph, group: LicenseGroup) -> bool:
        ...
        # Sprawdź czy owner nadal istnieje
        if owner not in graph.nodes():
            return False

        # Sprawdź czy wszyscy członkowie są sąsiadami ownera
