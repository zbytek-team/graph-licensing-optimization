"""
Common mutation operators for metaheuristic algorithms.
"""
...
from .solution_utils import SolutionBuilder


class MutationOperators:
    """Collection of mutation operators that can be reused across algorithms"""

    @staticmethod
    def apply_random_mutation(solution: Solution, graph: nx.Graph, license_types: List[LicenseType]) -> Optional[Solution]:
        """
        Apply a random mutation operator to the solution.
        ...
        Returns:
            Mutated solution or None if no valid mutation was found
        """
        operators = [
        ...
        # Try operators with equal probability
        weights = [0.3, 0.3, 0.2, 0.2]

        # Try multiple attempts to find a valid mutation
        for _ in range(10):
    ...
    def change_license_type(solution: Solution, graph: nx.Graph, license_types: List[LicenseType]) -> Optional[Solution]:
        ...
        Returns:
            Solution: Mutated solution or None if mutation not possible
        """
    ...
    def local_search_optimization(solution: Solution, graph: nx.Graph, license_types: List[LicenseType]) -> Solution:
        """
        Local search mutation that tries to improve the solution
        ...
        Returns:
            Solution: Improved solution or original if no improvement found
        """
        best_solution = solution

        # Try to improve any group by changing its license type
        for i, group in enumerate(solution.groups):
            ...
            for license_type in compatible:
                if license_type.cost < group.license_type.cost:
                    # Create new solution with cheaper license
        ...
        # Try to merge expensive small groups
