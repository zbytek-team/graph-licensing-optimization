"""Funkcje pomocnicze dla algorytmów: solution builder.
...
class SolutionBuilder:
        ...
        for license_type in license_types:
            if license_type.min_capacity <= combined_size <= license_type.max_capacity:
                for potential_owner in combined_members:
                    owner_neighbors = SolutionBuilder.get_owner_neighbors_with_self(
                        graph, potential_owner
                    )
                    if combined_members.issubset(owner_neighbors):
                        additional_members = combined_members - {potential_owner}
                        return LicenseGroup(
                            license_type, potential_owner, additional_members
                        )
        return None
    ...
    def find_cheapest_license_for_size(
        size: int, license_types: List[LicenseType]
    ) -> Optional[LicenseType]:
        compatible = [
            lt for lt in license_types if lt.min_capacity <= size <= lt.max_capacity
        ]
        if not compatible:
            return None
    ...
    @staticmethod
    def build_solution_for_subset(
        nodes: List, graph: nx.Graph, license_types: List[LicenseType]
    ) -> Solution:
