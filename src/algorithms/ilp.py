"""Moduł implementuje algorytm ilp dla dystrybucji licencji.
...
class ILPSolver(Algorithm):
    ...
    def solve(
        ...
        for i in nodes:
            for t_idx, license_type in enumerate(license_types):
                group_active[i, t_idx] = pulp.LpVariable(
                    f"group_active_{i}_{t_idx}", cat="Binary"
                )
        total_cost = pulp.lpSum(
            [
                group_active[i, t_idx] * license_type.cost
                for i in nodes
                for t_idx, license_type in enumerate(license_types)
            ]
        )
        ...
        if prob.status != pulp.LpStatusOptimal:
            raise RuntimeError(
                f"ILP solver failed with status: {pulp.LpStatus[prob.status]}"
            )
        groups = []
        covered_nodes = set()
        for i in nodes:
            for t_idx, license_type in enumerate(license_types):
                if (
                    group_active[i, t_idx].varValue
                    and group_active[i, t_idx].varValue > 0.5
                ):
                    group_members = set()
                    for j in nodes:
                        if (
                            (i, j, t_idx) in x
                            and x[i, j, t_idx].varValue
                            and x[i, j, t_idx].varValue > 0.5
                        ):
                            group_members.add(j)
                    if group_members:
        ...
        return Solution(
            groups=groups, total_cost=total_cost_value, covered_nodes=covered_nodes
        )
