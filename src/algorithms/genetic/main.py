...
class GeneticAlgorithm(Algorithm):
    ...
    def solve(self, graph: nx.Graph, license_types: List[LicenseType], **kwargs: Any) -> Solution:
        ...
        population = self.population_manager.initialize_population(graph, license_types, population_size)
        ...
        for generation in range(max_generations):
            ...
            if current_best.total_cost < best_solution.total_cost:
                ...
                if generations_without_improvement > 8 and generation % 3 == 0:
                    improved_best = self.operators.intensive_local_search(best_solution, graph, license_types)
            ...
            while len(new_population) < population_size:
                parent1 = self.operators.tournament_selection(population, fitness_scores)
                parent2 = self.operators.tournament_selection(population, fitness_scores)

                if random.random() < crossover_rate:
                    child1, child2 = self.operators.crossover(parent1, parent2, graph, license_types)
                else:
                    child1, child2 = parent1, parent2

                if random.random() < adaptive_mutation_rate:
                    mutated_child1 = self.operators.mutate(child1, graph, license_types)
                    if mutated_child1 and self.validator.is_valid_solution(mutated_child1, graph):
                        child1 = mutated_child1

                if random.random() < adaptive_mutation_rate:
                    mutated_child2 = self.operators.mutate(child2, graph, license_types)
                    if mutated_child2 and self.validator.is_valid_solution(mutated_child2, graph):
                        child2 = mutated_child2

                if child1 and self.validator.is_valid_solution(child1, graph):
                    new_population.append(child1)
                if child2 and self.validator.is_valid_solution(child2, graph) and len(new_population) < population_size:
            ...
            while len(new_population) < population_size:
                ...
                while attempts < max_attempts and len(new_population) < population_size:
                    solution = self.population_manager.generate_truly_random_solution(graph, license_types)
                ...
                if len(new_population) < population_size:
                    ...
                    attempts = 0
                    while attempts < max_attempts and len(new_population) < population_size:
                        mutated = self.operators.intensive_mutate(base_solution, graph, license_types)
        ...
        if not self.validator.is_valid_solution(best_solution, graph):
            repaired_solution = self.operators._force_repair_solution(best_solution, graph, license_types)
