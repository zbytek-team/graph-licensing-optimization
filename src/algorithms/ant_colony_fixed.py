...
class AntColonyOptimization(Algorithm):
    ...
    def solve(self, graph: nx.Graph, license_types: List[LicenseType]) -> Solution:
        ...
        # Initialize pheromone trails and heuristics
    ...
    def _construct_ant_solution(
        ...
        while uncovered:
            ...
            if len(group_nodes) >= license_type.min_capacity:
                ...
                valid_licenses = [lt for lt in license_types if lt.min_capacity <= 1 and lt.max_capacity >= 1]
                if valid_licenses:
                    best_license = min(valid_licenses, key=lambda lt: lt.cost)
                    group = LicenseGroup(best_license, start_node, set())
                    groups.append(group)
    ...
    def _build_connected_group(
        ...
        """Build a connected group starting from a node"""
        group = {start_node}

        while len(group) < license_type.max_capacity:
            # Find candidates: neighbors of current group that are uncovered
            candidates = set()
            for member in group:
                for neighbor in graph.neighbors(member):
                    if neighbor in uncovered and neighbor not in group:
                        candidates.add(neighbor)

            if not candidates:
                break

            # Choose next node to add probabilistically
            probabilities = {}
            for candidate in candidates:
                # Use pheromone from edge to any group member
                max_pheromone = 0.0
                for member in group:
                    edge_key = tuple(sorted([member, candidate]) + [license_type.name])
                ...
                probabilities[candidate] = (max_pheromone**self.alpha) * ((1.0 + eta) ** self.beta)
            ...
            if selected:
                ...
                if not nx.is_connected(subgraph):
                    # Remove if it breaks connectivity (shouldn't happen with our construction)
    ...
    def _is_valid_solution(self, solution: Solution, graph: nx.Graph) -> bool:
        """Check if solution is valid using the validator"""
        return SolutionValidator().is_valid_solution(solution, graph)
