"""Moduł implementuje algorytm ant colony dla dystrybucji licencji.
...
class AntColonyOptimization(Algorithm):
        ...
        for _ in range(self.max_iterations):
            ...
            for _ in range(self.num_ants):
                solution = self._construct_ant_solution(
                    graph, license_types, pheromones, heuristics
                )
    ...
    def _calculate_heuristics(
        ...
        for node in graph.nodes():
            ...
            for lt in license_types:
                efficiency = (
                    (lt.max_capacity / lt.cost) if lt.cost > 0 else float("inf")
                )
    ...
    def _construct_ant_solution(
        ...
        groups = []
        while uncovered:
            start_node = self._select_node(
                uncovered, license_types, pheromones, heuristics
            )
            if start_node is None:
                start_node = list(uncovered)[0]

            license_type = self._select_license(
                start_node, license_types, pheromones, heuristics
            )

            group_nodes = {start_node}
            candidates = list(
                set(graph.neighbors(start_node)) & uncovered - {start_node}
            )

            while len(group_nodes) < license_type.max_capacity and candidates:
                next_member = self._select_member(
                    start_node, candidates, license_type, pheromones, heuristics
                )
            ...
            if len(group_nodes) >= license_type.min_capacity:
                groups.append(
                    LicenseGroup(license_type, start_node, group_nodes - {start_node})
                )
                uncovered -= group_nodes
            else:
                cheapest_single = SolutionBuilder.find_cheapest_single_license(
                    license_types
                )
    ...
    def _select_component(
        self, choices: List[Any], probabilities: Dict[Any, float]
    ) -> Any:
        if not choices:
            return None
    ...
    def _get_choice_probabilities(
        self, choices: List[Any], pheromones: Dict, heuristics: Dict, key_func
    ) -> Dict[Any, float]:
        probabilities = {}
        for choice in choices:
            key = key_func(choice)
    ...
    def _select_node(
        ...
        for node in nodes:
            avg_prob = (
                sum(
                    (pheromones.get((node, lt.name), 1.0) ** self.alpha)
                    * (heuristics.get((node, lt.name), 1.0) ** self.beta)
                    for lt in license_types
                )
                / len(license_types)
                if license_types
    ...
    def _select_license(
        ...
        probabilities = self._get_choice_probabilities(
            license_types, pheromones, heuristics, lambda lt: (node, lt.name)
        )
    ...
    def _select_member(
        ...
        probabilities = self._get_choice_probabilities(
            ...
            pheromones,
            heuristics,
            lambda c: tuple(sorted((owner, c))) + (license_type.name,),
        )
        return self._select_component(candidates, probabilities)

    def _update_pheromones(
        self, pheromones: Dict, best_solution: Solution, graph: nx.Graph
    ):
    ...
    def _deposit_pheromones_for_solution(
        self, pheromones: Dict, solution: Solution, graph: nx.Graph
    ):
            ...
            for i, group in enumerate(groups):
                ...
                for new_license_type in sorted_licenses:
                    if (
                        ...
                        groups[i] = LicenseGroup(
                            new_license_type, group.owner, group.additional_members
                        )
                        current_solution = SolutionBuilder.create_solution_from_groups(
                            groups
                        )
                        improved = True
                        break
    ...
    def _generate_random_initial_solution(
        self, graph: nx.Graph, license_types: List[LicenseType]
    ) -> Solution:
