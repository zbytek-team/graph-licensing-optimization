...
from glopt.core.io.data_loader import RealWorldDataLoader
...
def _run_one(
    algo: str, graph: nx.Graph, lic: str, seed: int
) -> tuple[dict[str, object], bool]:
    parent_conn, child_conn = mp.Pipe(duplex=False)
    p = mp.Process(
        target=_worker_solve,
        args=(algo, graph, lic, seed, child_conn),
    )
    p.start()
            ...
            for algo in algorithm_classes:
                ...
                for ego_id in ego_ids:
                    ...
                    for rep in range(REPEATS_PER_GRAPH):
                        algo_seed = 12345 + rep
                        res = _run_one(algo, G, lic, seed=algo_seed)
                        row = {
                            "run_id": run_id,
                            "algorithm": algo,
                            "graph": "facebook_ego",
                        ...
                        if row.get("notes") == "timeout":
                            ...
                            status = "ERROR"
                        print(
                            f"{'facebook_ego':12s} ego={str(ego_id):8s} " \
                            f"rep={rep} cost={row['total_cost']:.2f} " \
