...
class NaiveAlgorithm(Algorithm):
    ...
    def solve(
        ...
        **kwargs: Any,
        ...
        for assignment in self._generate_all_assignments(
            nodes, graph, license_types
        ):
            if not assignment:
                continue
        ...
        if best is None:
            cheapest_single = min(
                license_types,
                key=lambda lt: lt.cost
                if lt.min_capacity <= 1 <= lt.max_capacity
                else float("inf"),
            )
            groups = [
                LicenseGroup(cheapest_single, node, frozenset())
                for node in nodes
            ]
    ...
    def _generate_all_assignments(
        ...
        for partition in self._generate_partitions(nodes):
            yield from self._generate_assignments_for_partition(
                partition, graph, license_types
            )

    def _generate_partitions(
        self, nodes: list[Any]
    ) -> Iterator[list[set[Any]]]:
        n = len(nodes)
        if n == 0:
            yield []
            return
    ...
    def _is_valid_group(
        self, owner: Any, members: set[Any], graph: nx.Graph
    ) -> bool:
    ...
    def _create_solution_from_assignment(
        ...
        groups = [
            LicenseGroup(lt, owner, frozenset(members))
            for lt, owner, members in assignment
        ]
        return SolutionBuilder.create_solution_from_groups(groups)
