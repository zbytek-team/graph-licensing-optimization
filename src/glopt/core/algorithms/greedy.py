...
class GreedyAlgorithm(Algorithm):
    ...
    def solve(
        ...
        **_: Any,
        ...
        licenses = sorted(
            license_types, key=lambda lt: (-lt.max_capacity, lt.cost)
        )
        ...
        for owner in sorted(nodes, key=lambda n: int(degv[n]), reverse=True):
            if owner not in uncovered:
                continue
            avail = (
                SolutionBuilder.get_owner_neighbors_with_self(graph, owner)
                & uncovered
            )
            if not avail:
                continue
            best_group = self._best_group_for_owner(
                owner, avail, graph, licenses, degv
            )
        ...
        while uncovered:
            ...
            fallback = self._cheapest_feasible_group(
                owner, avail, graph, license_types, degv
            )
    ...
    def _best_group_for_owner(
        ...
        for lt in licenses:
            ...
            grp = LicenseGroup(
                license_type=lt,
                owner=owner,
                additional_members=frozenset(additional),
            )
    ...
    def _cheapest_feasible_group(
        ...
        for lt in sorted(
            license_types, key=lambda x: (x.cost, -x.max_capacity)
        ):
            if len(avail) < lt.min_capacity:
                continue
            need_additional = max(0, lt.min_capacity - 1)
            pool = sorted(
                (n for n in avail if n != owner),
                key=lambda n: int(degv[n]),
                reverse=True,
            )
            ...
            return LicenseGroup(
                license_type=lt,
                owner=owner,
                additional_members=frozenset(chosen),
            )
        return None
