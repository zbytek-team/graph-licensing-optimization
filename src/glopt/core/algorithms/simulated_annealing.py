...
class SimulatedAnnealing(Algorithm):
        ...
        **kwargs: Any,
        ...
        for _ in range(max_iterations):
            ...
            if neighbor is None:
                ...
                if d < 0 or random.random() < math.exp(
                    -d / max(temperature, 1e-12)
                ):
                    current = neighbor
    ...
    def _mv_change_license(
        ...
        compat = SolutionBuilder.get_compatible_license_types(
            g.size, lts, exclude=g.license_type
        )
        ...
        new_groups = [
            LicenseGroup(new_lt, g.owner, g.additional_members)
            if x is g
            else x
            for x in solution.groups
        ]
    ...
    def _mv_move_member(
        ...
        donors = [
            ...
            if g.additional_members and g.size > g.license_type.min_capacity
        ]
        if not donors:
            return None
        ...
        receivers = [
            g
            for g in solution.groups
            if g is not from_g and g.size < g.license_type.max_capacity
        ]
        if not receivers:
            return None
        to_g = random.choice(receivers)
        allowed = SolutionBuilder.get_owner_neighbors_with_self(
            graph, to_g.owner
        )
    ...
    def _mv_swap_members(
        ...
        if n1 not in SolutionBuilder.get_owner_neighbors_with_self(
            graph, g2.owner
        ):
            return None
        if n2 not in SolutionBuilder.get_owner_neighbors_with_self(
            graph, g1.owner
        ):
            return None
    ...
    def _mv_split_group(
        ...
        for _ in range(4):
            ...
            lt1 = SolutionBuilder.find_cheapest_license_for_size(
                len(part1), lts
            )
            lt2 = SolutionBuilder.find_cheapest_license_for_size(
                len(part2), lts
            )
            ...
            neigh1 = SolutionBuilder.get_owner_neighbors_with_self(
                graph, owner1
            )
            neigh2 = SolutionBuilder.get_owner_neighbors_with_self(
                graph, owner2
            )
            if not set(part1).issubset(neigh1) or not set(part2).issubset(
                neigh2
            ):
                continue
            new_groups = [x for x in solution.groups if x is not g]
            new_groups.append(
                LicenseGroup(lt1, owner1, frozenset(set(part1) - {owner1}))
            )
            new_groups.append(
                LicenseGroup(lt2, owner2, frozenset(set(part2) - {owner2}))
            )
            return Solution(groups=tuple(new_groups))
        return None
