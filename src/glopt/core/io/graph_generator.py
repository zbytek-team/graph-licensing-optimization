...
class GraphGeneratorFactory:
    _GENERATORS: ClassVar[dict[str, GeneratorFn]] = {
        "random": lambda *, n_nodes, **p: GraphGeneratorFactory._random(
            n_nodes, **p
        ),
        "scale_free": lambda *,
        n_nodes,
        **p: GraphGeneratorFactory._scale_free(n_nodes, **p),
        "small_world": lambda *,
        n_nodes,
        **p: GraphGeneratorFactory._small_world(n_nodes, **p),
        "complete": lambda *, n_nodes, **p: GraphGeneratorFactory._complete(
            n_nodes, **p
        ),
        "star": lambda *, n_nodes, **p: GraphGeneratorFactory._star(
            n_nodes, **p
        ),
        "path": lambda *, n_nodes, **p: GraphGeneratorFactory._path(
            n_nodes, **p
        ),
        "cycle": lambda *, n_nodes, **p: GraphGeneratorFactory._cycle(
            n_nodes, **p
        ),
        "tree": lambda *, n_nodes, **p: GraphGeneratorFactory._tree(
            n_nodes, **p
        ),
    }
    ...
    @staticmethod
    def _random(
        n_nodes: int, *, p: float = 0.1, seed: int | None = None
    ) -> nx.Graph:
        return nx.gnp_random_graph(n=n_nodes, p=p, seed=seed)

    @staticmethod
    def _scale_free(
        n_nodes: int, *, m: int = 2, seed: int | None = None
    ) -> nx.Graph:
        if n_nodes <= 1:
